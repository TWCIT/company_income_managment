                    ...
                    ToolbarModelView(title: getToolbarTitle(), button1Icon: IMAGE_FILTER_ICON) { self.presentationMode.wrappedValue.dismiss() }
                        ...
                        .actionSheet(isPresented: $showingSheet) {
                            ActionSheet(title: Text("Wybierz filtr"), buttons: [
                                    .default(Text("Całość")) { filter = .all },
...
struct ExpenseFilterChartView: View {
    ...
    var body: some View {
        Group {
            if !expense.isEmpty {
                ChartView(label: "Całkowity \(isIncome ? "Przychód" : "Wydatek") - \(CURRENCY)\(getTotalValue())",
