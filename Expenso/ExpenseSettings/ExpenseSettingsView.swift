    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                VStack {
                    ToolbarModelView(title: "Ustawienia") { self.presentationMode.wrappedValue.dismiss() }
                    ...
                    VStack {
                        ...
                        */
                        Button(action: { selectCurrency = true }, label: {
                            HStack {
                                Spacer()
                                TextView(text: "Waluta - \(viewModel.currency)", type: .button).foregroundColor(Color.text_primary_color)
                                Spacer()
                            }
                        })
                        .frame(height: 25)
                        ...
                        .actionSheet(isPresented: $selectCurrency) {
                            ...
                            return ActionSheet(title: Text("Wybierz walutę"), buttons: buttons)
                        }
                        /*
